# enhanced_server_with_database.py - With Detailed Logging
import socket
import threading
import json
import time
        ...
        try:
            ...
            if requester in self.clients:
                try:
                    ...
                    log_networking(f"Failed to send message history to {requester}", requester)
    ...
    def create_group(self, creator, group_name, members):
        ...
        if self.db.create_group(group_name, creator):
            ...
            for member in valid_members:
                ...
                if member in self.clients:
                    try:
                        ...
                        log_networking(f"Failed to send group info to {member}")
            ...
            if invalid_members:
                ...
                log_networking(f"⚠️ Invalid members in group creation: {invalid_list}")
        else:
    ...
    def send_personal_message(self, sender, recipient, content):
        ...
        if recipient in self.clients:
            ...
            try:
                ...
                log_networking(f"❌ Failed to deliver personal message to {recipient}")
                if sender != "SERVER":
                    try:
                        sender_socket = self.clients[sender][0]
